...
//         // Replace the last timestamp in the requestTimeAndDate array or add a new one if the array is empty
//         if (dustbin.requestTimeAndDate.length > 0) {
//             dustbin.requestTimeAndDate[dustbin.requestTimeAndDate.length - 1] = new Date(); // Replace last timestamp
//         } else {
//             dustbin.requestTimeAndDate = [new Date()]; // Set the current date if empty
//         }
...
exports.addDustbin = async (req, res) => {
    ...
    try {
        const newDustbin = new Dustbin({
            ...
            dustbinName,
            responsiblePerson,
            requestTimeAndDate: new Date() // Set the current date
        });
        const savedDustbin = await newDustbin.save();
        res.status(201).json(savedDustbin);
    } catch (error) {
